...
- `source-catalog/PC2_INVERTER_WARNING_SOURCES.md` — focused source record for the mission-specific inverter-2 starting configuration, inverter-caution path, exact alternate-inverter transfer procedure, fresh-valid-reobservation timing boundary, source-backed ground electrical-observation path through `GC0155`/`GC0071`, and note-117 direct caution/selector telemetry boundary; no unsupported numeric dwell, direct ground caution discrete, selector telemetry, or exact controller CRT routing is assumed.
...
- `research/111_pc2_inverter_switch_identity.md` — records the earlier generic LM inverter-convention synthesis; superseded in part by mission-specific evidence in notes 112–117.
