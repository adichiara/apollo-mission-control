...
| NASA NTRS 19700004489, table reproducing GAEC specification LSP-470-2D, *Master End Item Specification for Lunar Module* | Landing-radar range and velocity accuracy requirements stated as 3-sigma limits; range: `1.4% + 15 ft` at 2,000–25,000 ft and `1.4% + 5 ft` at 10–2,000 ft; altitude-banded component velocity limits also given | Quantitative **performance/acceptance envelope**. Do not infer Gaussian historical noise merely because limits are expressed at 3 sigma. |
...
The landing-radar numerical-error thread is now narrower. NTRS 19700004489 reproduces the GAEC LSP-470-2D master-end-item 3-sigma accuracy limits, so the project no longer lacks a quantitative radar performance envelope. However, this does **not** close historical stochastic generation. TN D-6849's LM-5 test history specifically records that a Gaussian assumption used for Doppler-spectrum-simulator limits required correction because of heavier tails. A 3-sigma specification therefore must not be converted into a Gaussian random-error model without additional evidence.

MSC-69-EG-14 / NASA-TM-X-64374 remains useful as an adjacent-effectivity retrieval target for additional test detail, but it is no longer the sole quantitative lead.
...
- **DOCUMENTED:** GAEC LSP-470-2D 3-sigma landing-radar accuracy envelope as reproduced in NASA primary material; not a stochastic distribution.
