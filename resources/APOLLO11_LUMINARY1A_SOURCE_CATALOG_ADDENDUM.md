...
| Rozas & Cunningham, NASA TN D-6849 / MSC-S-311, *Apollo Experience Report: Lunar Module Landing Radar and Rendezvous Radar*, Jun 1972 | LR design/test history; LM-5 one-count velocity-bias defect corrected preflight; Doppler-spectrum-simulator Gaussian test assumption required heavier-tail correction; Apollo 11 LR flight data appeared within specification except a few low-velocity points near zero Doppler where tracking was not expected; two questionable points were probably caused by poor data processing during the LGC overload alarm | Primary NASA Apollo-program experience authority. Supports a localized Apollo-11 flight-behavior boundary and the distinction between specification envelope and stochastic model. Does **not** supply a numerical flight-effective probability distribution, dropout rate, correlation model, or controller-display behavior. |
...
The landing-radar numerical-error thread is narrower. NTRS 19700004489 reproduces the GAEC LSP-470-2D master-end-item 3-sigma accuracy limits, including the exact altitude/component velocity cells and the `whichever is greater` rule. TN D-6849 then supplies mission-effective observed behavior: Apollo 11 LR data were generally within specification, with a localized low-velocity/near-zero-Doppler region where tracking was not expected and two questionable samples probably associated with poor processing during the LGC overload alarm. This is useful validation evidence, but it still does **not** close historical stochastic generation. The same TN records that a Gaussian assumption used for Doppler-spectrum-simulator limits required correction because of heavier tails. Neither a 3-sigma requirement nor the observed localized exception may be converted into a Gaussian random-error model.
...
- **DOCUMENTED, APOLLO 11 FLIGHT EXPERIENCE:** TN D-6849 establishes generally in-spec LR behavior plus the localized near-zero-Doppler/LGC-overload processing exception; not a probability model.
...
- **BLOCKED:** direct PHO-TN401 inspection; stochastic Apollo-11-effective LR error distribution/process beyond the documented 3-sigma performance envelope and localized flight exception.
