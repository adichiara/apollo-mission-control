...
The Apollo 11 profile now carries the exact LM-5 position-1/position-2 angle loads in revolutions and exposes a provenance-bearing adapter that converts the selected pair to radians and combines it with explicit measurement-time CDUs. This feeds the already verified SETPOS/NBSM estimator chain without manual historical-constant transcription.
