...
| Virtual AGC document-library PCR catalog | Describes PCR-700A as a rewritten clarification of PCR-700 | Secondary retrieval aid only; seek primary PCR/change-control artifact before treating wording as technically identical. |

## Effectivity lesson from research 331–333

R-567 revision numbers are section-specific. Section 5 Revision 5 remains the concrete descriptive-equation retrieval target, while later LUMINARY 1B/1C sources are comparison endpoints. However, the LUMINARY 099 assembly listing now independently closes the exact P66 ROD computation interval: the final program schedules `RODTASK` at `1SEC`. Section 5/`69-FS-3` recovery is no longer required to establish that interval.

## Retrieval priority

1. Recover Section 5 Revision 5 landing-program/P63-P66 affected pages when needed for descriptive equation/page ancestry.
2. Recover a primary PCR-700/PCR-700A crosswalk.
3. Recover `69-FS-3` as the preferred complete LUMINARY 1A programmed-guidance-equation source.
4. If crew-visible Noun 63 timing becomes material, research the DSKY display-service path separately.
5. If station-visible timing becomes material, establish downlink and ground-display ancestry separately before changing controller products.
6. Do not spend further research effort reconfirming the one-second P66 ROD computation cadence absent contradictory primary evidence.
