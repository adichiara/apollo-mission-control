...
**PARTIALLY DOCUMENTED.** Design envelope, LM-5 launch/unmanned and transfer bookkeeping, powered-descent telemetry relationships, reconstruction methodology, and descent validation/checkpoint products are documented. The mass-properties source family is identified, but the public Rev. 2 scan demonstrably contains later amendment replacements, so the exact LM-5 sequential row still requires a Mission-G-provenanced page rather than blind extraction by table number. The official BET Volume II and Apollo 11 DPS Supplement 7 remain **BLOCKED ON NAMED SOURCE RECOVERY**. Exact delivered thrust/Isp remain unresolved.