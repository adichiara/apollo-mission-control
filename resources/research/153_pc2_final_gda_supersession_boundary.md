...
The ~59-hour trim pair cannot have survived unchanged into PC+2 ignition. The restored air-ground record at GET ~59:03 carried an interim pair of `5.86°` and `6.75°` and explicitly said the angles would be updated. The mission-specific LM CONTROL postflight report now independently bounds the later roll-axis state: immediately before PC+2 ignition the roll GDA was approximately `-0.8°`, then moved to approximately `-2°` as the engine responded at ignition.

Therefore the historical product lifecycle contains at least:

`~59 h interim trim (update expected) -> later superseding trim/state -> ~79:27 pre-ignition roll GDA ≈ -0.8° -> ignition compliance response ≈ -2°`

The complete later trim pair is still not recovered.

## Axis-label correction

Research note 152 and several derivative project documents called the second ~59-hour value a **yaw** trim. That is too strong. The air-ground exchange itself is confused: CAPCOM initially describes the values generically, Lovell suggests pitch/yaw, and Haise's final accepted readback gives pitch and roll. More importantly, the mission-specific LM CONTROL report describes the relevant DPS actuator as the **roll GDA**. Project documentation should therefore record the interim pair conservatively as `pitch 5.86° / roll 6.75°` (or simply the two DPS GDA values) rather than normalize the second value to yaw.

## Boundary retained

Do not infer:

- the final pitch-GDA value;
- that `-0.8°` was a pad-printed value rather than the actual pre-ignition actuator state;
...
The arithmetic `-0.8°` is an approximate value derived directly from the report's approximate `-2°` state and `-1.2°` delta.

## Simulation implication

Represent commanded/pad trim separately from observed actuator state and engine-compliance response. A maneuver product may be superseded before execution, and ignition can produce a further physical GDA displacement without implying a new targeting product.

## Next archival target

Recover the final ~78-hour P30/GDA pad or Flight Director/Flight Dynamics working sheet that gives the complete superseding PC+2 trim pair and its provenance. The mass-properties job number and explicit `T+55` relationship remain unresolved.