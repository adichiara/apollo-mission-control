...
Research note 236 resolves the modern request path for the E.155B1 target without treating the preliminary inventory's legacy location as a current shelf address.
...
A 1973 Singer/Link technical publication lists `P.O. 2-24481C (NASA-1100)` with customer `Grumman A/C Corp. (NASA)` and application `LEM`:
https://www.bitsavers.org/pdf/singer/link/Link_Training_Systems_by_Singer_197311.pdf

NASA/NARA's corporate index independently places the `LMA-790-2-LMS` handbook family under `NAS9-1100`:
https://www.ibiblio.org/apollo/NARASWoverflow/CorporateIndex.pdf

Use: independently corroborates that the purchase-order number reported on the `1L5-102-(H)` title page belongs to the Grumman/NASA LEM simulator program context. Do not silently normalize Singer's `NASA-1100` rendering to `NAS9-1100`; preserve both source forms.
...
## Current surviving-copy access status

As of 17 September 2026, the Heritage listing identifies the surviving physical `1L5-102-(H)` manual as Lot 71524 in its **26 September 2026 Space Exploration Showcase Auction #33174**. The listing advertises a highest-resolution image interface, but the searchable/opened web representation inspected for research note 225 did not expose internal acceptance-test pages.

Use: current custody/access locator only. The auction listing remains secondary evidence about technical contents. No acceptance criterion, observable, kit scope, effectivity, station mapping, or Apollo 13 applicability is promoted from it.
