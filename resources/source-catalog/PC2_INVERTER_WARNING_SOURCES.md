...
- **First-playable consequence:** normal selection-transient suppression belongs to spacecraft indication logic. It does not justify inventing a separate crew persistence timer after the transfer.
- **Caution:** does not establish a numeric Apollo 13 inhibit duration, the exact Apollo 13 PC+2 controller CRT field, telemetry word, ground display latency, or ground visibility of switch position.
...
The initial inverter-2 identity is mission-specific and directly supported. The alternate inverter-1 identity is supported by the mission-specific starting state plus the two-inverter architecture. The exact transfer sequence is directly supported by the Apollo 13 LM Malfunction Procedures. The remaining timing rule is now bounded as **valid-state re-observation rather than a fabricated crew dwell**. A numeric inhibit duration, crew-member assignment, exact controller wording, selector-position ground visibility, and exact TELMU/CONTROL field remain unresolved and must not be invented.
