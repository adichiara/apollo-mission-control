...
Status: active source supplement for the first-playable final state-vector/target-load workflow; staged executable implementation added in research note 099.
...
- **Implementation consequence:** The earlier approximately 75:35 state-vector/target load is treated as completed context at the 77:55 scenario start; later final-preparation work is modeled separately.
...
- **Implementation consequence:** Uplink configuration, transmission state, completion/computer return, and ranging are represented as separate workflow concepts.
- **Limitation:** Air-ground does not expose all internal Mission Control computation/authorization steps, and the repository does not infer an exact transmission duration from the transcript.
...
- **Implementation consequence:** Station projections preserve separate FIDO/RETRO, GUIDO, INCO, CAPCOM, and FLIGHT workflow information.
...
- **Implementation consequence:** Approximate final-preparation fixture events are chronology anchors only; `~` does not claim second-level precision.
- **Limitation:** Timeline placement is coarse and does not establish exact sub-minute uplink timing by itself.

## Implementation rule

Model the final state-vector/target load as a staged information-transfer workflow across FIDO/RTCC, GUIDO, INCO, CAPCOM/crew, and FLIGHT. Current executable states are documented in research note 099. Do not invent exact Cartesian vector values, internal command strings, display IDs, controller key sequences, hidden load-correctness semantics, or transmission durations unless a primary source establishes them.