...
- a source-bounded attitude-error/rate rule component with historical nominal validation envelopes and synthetic threshold tests.
...
The next implementation target is to wire the source-bounded attitude-monitoring helper into the common CONTROL product projection and shutdown-rule audit path. Exact startup-transient timing will remain unresolved unless a directly relevant primary flight-rule/procedure/control source is recovered.
