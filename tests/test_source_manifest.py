...
class ManifestIntegrityTests(unittest.TestCase):
    ...
    def test_unclear_rights_are_link_only(self):
        """docs/SOURCE_MIRRORING.md: anything whose rights basis is not clear is
        link-only. A note explaining the doubt does not license mirroring it."""
        for doc in self.documents:
            if doc["rights"] == "unclear":
                with self.subTest(doc=doc["id"]):
                    self.assertEqual(
                        doc["tier"],
                        "link-only",
                        f"{doc['id']} has unclear rights and must not be mirrored",
                    )

    def test_link_only_entries_explain_themselves(self):
        """A link-only entry should say why it is not mirrored, so the choice is
        reviewable rather than looking like an oversight."""
        for doc in self.documents:
            if doc["tier"] == "link-only":
                with self.subTest(doc=doc["id"]):
                    self.assertTrue(doc.get("note"), f"{doc['id']} needs a note")
