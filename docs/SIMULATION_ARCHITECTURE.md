...
The cross-scenario fixture metadata contract currently includes scenario identity/title, mission, provenance/status class, runtime-adapter identity, mission-profile identity, mission-time bounds, and vehicle configuration. PC+2-specific state remains below that boundary. Research note 162 now supplies the second reference case used to judge what additional state should become common.
