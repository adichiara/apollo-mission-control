...
3. Continue targeted research into Apollo LMS/AMS mathematical-model, subsystem-model, malfunction-insertion, instructor, output-dictionary, and validation/correlation documentation. Research note 215 fixes an AMS architecture boundary: dynamics, vehicle systems, simulator effects, simulator control, and MCC interface were distinct program classes; physical/system failures and telemetry faults could be inserted at different layers. Research note 219 now locates NASA RG 255 series E.155B1, a 1968–1970 Lunar Module Simulator project-file series explicitly containing acceptance test plans and procedures. The numerical criteria themselves remain unrecovered, so this is a concrete archival target rather than permission to invent historical tolerances.
...
9. Continue the player-interaction/playability workstream from research note 313: audit test-surface leakage, prototype FLIGHT/CAPCOM interaction structure, define phone product-density options, and instrument packet/interface findability. Keep final historical station reconstruction and seven-seat/five-player PASS claims deferred until the causal engine/simulated-crew loop and the relevant product boundaries are sufficiently mature.
