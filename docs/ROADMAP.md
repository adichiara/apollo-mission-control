...
The primary research chain now runs through notes **048–128**. Notes 098–103 establish the final-load, support-room, post-burn, spacecraft-model, and ground-data-processing boundaries. Notes 104–128 further constrain the PC+2 observation and shutdown-rule evidence paths, including inlet-pressure and thrust-monitor semantics, attitude-transient handling, inverter transfer/re-observation, ground inverter telemetry, TELMU station continuity, crew-local electrical monitoring, H-2 PHO-TR155 provenance/implementation, mission-specific H-2 TDFCB telemetry configuration, PHO-TR155 data-pack lineage/process boundaries, and the organizational-acronym search boundary.
...
For TELMU inverter monitoring, source-backed `GC0071V` / `GC0155F` evidence may be rendered as a project product. H-2 has two identified mission-specific configuration authorities: PHO-TR155 Revision C for the Display System and TDFCB Revision 4 for telemetry-format/Flight Control products. Their cross-check is documented. PHO-TR155 also used supporting data-pack inputs: PHO-TR515 establishes that data-pack circulation fed the Requirements and Configuration process that prepared PHO-TR155, while preliminary PHO-TR155 working lists fed display production. The surviving H-2 data-pack Revision N reference therefore does not establish which input pack contained TELMU loading or that Revision N itself was a console-loading sheet. Contemporary branch acronyms/names can be used to improve archival searches, but note 128 forbids treating those matches as direct definitions of the PHO-TR460 pack labels without an explicit cross-reference. Exact Apollo 13 indicator/module placement, CRT request, selector workflow, sample/display cadence, precision, and latency remain unresolved.
...
Research notes **125–128** add a historical configuration-control boundary above this functional model. Mission H-2 TDFCB Revision 4 carried mission-specific telemetry-format/Flight Control products and its PCMGS material was checked against PHO-TR155. PHO-TR155 itself was prepared by Requirements and Configuration from information gathered through data-pack circulation; preliminary PHO-TR155 working lists then supported display-production work. Because the contents/scope of H-2 Revision N are unrecovered, it cannot substitute for an exact TELMU loading source or be treated as synonymous with PHO-TR155 Revision C. Contemporary organizational acronym matches now improve the archival search vocabulary but still do not define the PHO-TR460 pack labels. Neither archival gap may be filled with guessed H-2 parameter loading or adjacent-mission sample rates.
...
9. Continue the archival search for **Mission H-2 data-pack Revision N or its transmittal/index**, a **Requirements and Configuration data-pack definition/key**, **PHO-TR155 Mission H-2 Revision C**, and **Mission H-2 TDFCB Revision 4** (especially special LM Flight Control and PCMGS listings). Include the directly attested `FCOB` / Flight Control Operations Branch and `LSB` / LM Systems Branch vocabulary, plus contemporary CSM Systems, Flight Dynamics, Mission Control Requirements, and Experiments Systems branch names as search leads only; do not assume they define the PHO-TR460 pack labels. Do not block physical validation on archival recovery.
...
Research notes 095–128 and associated testing/scope documentation improve physical-run evidence quality and scenario closure but do not constitute physical validation. Physical seven-seat and five-player compact human/device PASS claims remain unmade.
