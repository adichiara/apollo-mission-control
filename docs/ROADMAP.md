...
2. Continue integrating the deterministic simulated-crew actor. Live CAPCOM-driven actions and procedures briefed before the playable window now use separate reusable paths. The PC+2 premature-stop restart branch preserves eligibility → prebriefed crew procedure → explicit physical response without inventing a new post-stop FLIGHT/CAPCOM instruction.
