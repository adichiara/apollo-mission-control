...
The deployed nominal and ΔP paths are validated. Final player-interface reconstruction and multi-human playtesting are still pending, but **player-interaction/playability design is now an active parallel workstream** under research note 314. This permits interface auditing, prototype structure, and playability instrumentation without freezing unsupported historical display behavior. Research note 214 completed the first D-022 survey and found **no current PC+2 numerical gate with a valid fully sourced same-input range**; targeted archival recovery therefore remains the immediate historical-integration task in parallel.
...
2. Continue integrating the deterministic simulated-crew actor. Live CAPCOM-driven actions and procedures briefed before the playable window use separate reusable paths. The PC+2 ΔP shutdown, inverter-transfer contingency, and premature-stop restart branch now all have guided site-facing validation. The restart path preserves eligibility → prebriefed crew procedure → explicit physical response without inventing a new post-stop FLIGHT/CAPCOM instruction.
3. Continue targeted research into Apollo LMS/AMS mathematical-model, subsystem-model, malfunction-insertion, instructor, output-dictionary, and validation/correlation documentation. Research note 315 now raises the surviving LMS Volume II Section 2 **Malfunction Data** (three Avitabile folders) to a high-priority direct retrieval target; its title/extent are documented, but no malfunction content is executable until the pages are recovered and configuration-checked. Research note 215 fixes an AMS architecture boundary: dynamics, vehicle systems, simulator effects, simulator control, and MCC interface were distinct program classes; physical/system failures and telemetry faults could be inserted at different layers. Research note 219 now locates NASA RG 255 series E.155B1, a 1968–1970 Lunar Module Simulator project-file series explicitly containing acceptance test plans and procedures. The numerical criteria themselves remain unrecovered, so this is a concrete archival target rather than permission to invent historical tolerances.
4. Extract the original simulator's useful abstraction boundaries: state variables, equations, subsystem coupling, integration/update assumptions, failure insertion points, validation observables, and separation of physical truth from indications/telemetry. Research note 216 fixes the LMS source hierarchy/timestep boundary; note 217 adds direct LMS evidence for an explicit source-variable → measurement/output dictionary with spacecraft effectivity and telemetry-console malfunction handling; note 219 separates known existence of formal acceptance documentation from still-unrecovered acceptance/correlation criteria.
5. Keep D-022 ready but do not force a test without a valid range. Research note 214 found no current same-input sourced interval among the open PC+2 mass, propulsion, trim, trajectory, tracking, or acceptance-tolerance gates. Prioritize recovery of Apollo 13 DPS Supplement 2, LM-7 performance/calibration data, H-2 mass-property/depletion records, the final PC+2 GDA artifact, or source-backed product/tolerance data. When one supplies a genuine range, run the D-022 endpoint sensitivity against controller-visible products.
...
9. Continue the player-interaction/playability workstream from research note 314: audit test-surface leakage, prototype FLIGHT/CAPCOM interaction structure, define phone product-density options, and instrument packet/interface findability. Keep final historical station reconstruction and seven-seat/five-player PASS claims deferred until the causal engine/simulated-crew loop and the relevant product boundaries are sufficiently mature.
10. Continue the Apollo 11 powered-descent/program-alarm interval as the second architecture pressure test. The guidance alarm/restart domain, landing-radar update-eligibility gate, generic pairwise guidance cross-check machinery, generic multi-source consensus/quorum machinery, and non-executable scenario/model-profile metadata are implemented. These reusable domains now also have facilitator-facing model-proof surfaces in the Causal Model Lab. Research note 204 defines source-backed Apollo 11 guidance-monitoring source pairs and velocity-residual limits; note 210 adds the PGNCS/AGS/MSFN two-out-of-three topology. Research note 218 now recovers Mission G MSFN/PFP source-production timing: 10-Hz tracking input and expected 0.2/0.4-s processing/time-tag intervals. The Causal Model Lab exposes this profile and shows the historical execution gate as blocked. Do not reinterpret processor cadence as inter-source freshness: maximum permissible PGNCS/AGS/MSFN observation-time separation, controller display synchronization, and downstream operational response remain unresolved. The landing-radar path now includes a generic upstream measurement-qualification model plus Apollo 11 source-profile values for Data Good persistence, range-scale stability, update-state boundaries, and residual tests; remaining radar work is LM-5 measurement geometry/reference computation and the downstream estimator/filter. Then complete powered-descent trajectory/propulsion, controller-product interfaces, and sourced decision rules before an Apollo 11 runtime is allowed.
