...
4. Extract the original simulator's useful abstraction boundaries: state variables, equations, subsystem coupling, integration/update assumptions, failure insertion points, and separation of physical truth from indications/telemetry. Research note 216 now fixes the LMS source hierarchy and explicitly blocks treating the cataloged 50 ms AACS timestep study as a universal LMS timestep.
