...
9. Continue the player-interaction/playability workstream from research note 314. The FLIGHT/CAPCOM lab now preserves the full approval/transmission/receipt distinction. Run the neutral FLIGHT/CAPCOM part-task checkout under continuous GET, then prototype CONTROL/GUIDO phone product scanning using stable single-product layouts and general controller actions rather than branch-specific solution controls. Instrument packet/interface findability. Keep final historical station reconstruction and seven-seat/five-player PASS claims deferred until the causal engine/simulated-crew loop and relevant product boundaries are sufficiently mature.
