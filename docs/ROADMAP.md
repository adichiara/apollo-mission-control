...
1. Preserve the facilitator/test interface as the primary validation surface. The deployed Causal Model Lab now exposes DPS, trajectory/tracking, resource→power→observation, guidance comparison/consensus, landing-radar qualification/update gating, and guidance-computer alarm/restart proofs; new reusable causal domains should add their site-facing proof in the same workstream.
...
3. Continue targeted research into Apollo LMS/AMS mathematical-model, subsystem-model, malfunction-insertion, instructor, output-dictionary, and validation/correlation documentation. Research note 215 now fixes an AMS architecture boundary: dynamics, vehicle systems, simulator effects, simulator control, and MCC interface were distinct program classes; physical/system failures and telemetry faults could be inserted at different layers.
4. Extract the original simulator's useful abstraction boundaries: state variables, equations, subsystem coupling, integration/update assumptions, failure insertion points, and separation of physical truth from indications/telemetry. Research note 216 fixes the LMS source hierarchy/timestep boundary; note 217 now adds direct LMS evidence for an explicit source-variable → measurement/output dictionary with spacecraft effectivity and telemetry-console malfunction handling.
