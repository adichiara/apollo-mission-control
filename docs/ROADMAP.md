...
Mission H-2 display/telemetry provenance has now been tightened through research notes **123–129**. PHO-TR155 Revision C was issued before flight, the H-2 Display System was implemented in accordance with it during March 1970, and Philco explicitly reported that no equipment configuration changes were required. Separately, Mission H-2 **TDFCB Revision 4** was delivered on 28 January 1970 with mission-specific LM Flight Control, PCMGS, high-speed/wideband, index, and compare products; Philco records checking the H-2 Rev. 4 PCMGS against PHO-TR155. Earlier PHO-TR155 work also used multiple named H-2 data-pack products plus a preliminary IBM card deck/listing. PHO-TR515 establishes the process relationship: the Requirements and Configuration Section gathered requirements through **data-pack circulation** and prepared PHO-TR155, whose preliminary working lists then supported downstream display-production work. Research note 129 adds a separate Flight Control recorder-requirements path in which branch requirements carried console/measurement or console/subformat assignments and the Flight Control Data Requirements Document carried format/subformat, measurement, title, and pen-location information. This identifies a new mission-specific archival route without proving that any PHO-TR460 pack abbreviation denotes that document. Exact Apollo 13 TELMU loading remains unrecovered, so project renderings must not be represented as exact H-2 physical-console or parameter-loading reconstructions.
...
The primary research chain now runs through notes **048–129**. Notes 098–103 establish the final-load, support-room, post-burn, spacecraft-model, and ground-data-processing boundaries. Notes 104–129 further constrain the PC+2 observation and shutdown-rule evidence paths, including inlet-pressure and thrust-monitor semantics, attitude-transient handling, inverter transfer/re-observation, ground inverter telemetry, TELMU station continuity, crew-local electrical monitoring, H-2 PHO-TR155 provenance/implementation, mission-specific H-2 TDFCB telemetry configuration, PHO-TR155 data-pack lineage/process boundaries, the organizational-acronym search boundary, and the Flight Control recorder-requirements source path.
...
For TELMU inverter monitoring, source-backed `GC0071V` / `GC0155F` evidence may be rendered as a project product. H-2 has two identified mission-specific configuration authorities: PHO-TR155 Revision C for the Display System and TDFCB Revision 4 for telemetry-format/Flight Control products. Their cross-check is documented. PHO-TR515 adds two distinct process clues: data-pack circulation fed PHO-TR155 preparation, and Flight Control recorder requirements could carry console/measurement, format/subformat, and pen-location assignments. The latter makes an H-2 Flight Control Data Requirements Document or recorder cross-reference a high-value target, but does not establish that the inverter measurements were on a TELMU recorder or that a PHO-TR460 pack label maps to that document. Exact Apollo 13 indicator/module placement, CRT request, recorder assignment, selector workflow, sample/display cadence, precision, and latency remain unresolved.
...
Research notes **125–129** add a historical configuration-control boundary above this functional model. Mission H-2 TDFCB Revision 4 carried mission-specific telemetry-format/Flight Control products and its PCMGS material was checked against PHO-TR155. PHO-TR155 itself was prepared by Requirements and Configuration from information gathered through data-pack circulation; preliminary PHO-TR155 working lists then supported display-production work. PHO-TR515 also identifies a distinct recorder-production requirements path carrying console/measurement, format/subformat, title, and pen-location assignments. Because the H-2 versions of those materials are unrecovered, they cannot substitute for an exact TELMU loading source or justify a recorder assignment. Contemporary organizational acronym matches improve archival search vocabulary but still do not define the PHO-TR460 pack labels.
...
9. Continue the archival search for a **Mission H-2 Flight Control Data Requirements Document / branch requirement / recorder cross-reference**, **Mission H-2 data-pack Revision N or transmittal/index**, a **Requirements and Configuration data-pack definition/key**, **PHO-TR155 Mission H-2 Revision C**, and **Mission H-2 TDFCB Revision 4**. Treat directly attested branch names/acronyms only as search vocabulary; do not infer pack definitions or recorder assignments without H-2 evidence. Do not block physical validation on archival recovery.
...
- exact H-2 TELMU module/indicator/CRT loading, recorder assignment, request/key workflow, and display cadence without mission-specific evidence;
...
Research notes 095–129 and associated testing/scope documentation improve physical-run evidence quality and scenario closure but do not constitute physical validation. Physical seven-seat and five-player compact human/device PASS claims remain unmade.
