...
4. Reopen exact LMS retrieval only when a selected scenario/model names the dependency. The controlled routes are already known: Volume II Section 2 **Malfunction Data**, Section 6 **Scripting Data Sheets**, and Section 7 **Simulator Output Tables** in the Virginia Tech Avitabile collection; NASA RG 255 E.155B1 for acceptance material; and the large public Volume I/User's Manual scans for page-controlled extraction. Their unresolved contents remain cataloged without blocking current causal-engine work.
