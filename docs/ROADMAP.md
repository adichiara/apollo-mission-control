...
Notes 118–121 add four presentation constraints. Apollo computer-driven TV displays were dynamically requested and assigned to available channels, with channel-attach also supported; later LM-10 documentation proves `GC0071V`/`GC0155F` were operational telemetry products with format-dependent sampling and primary MSKs; the Apollo 13 AS-508 configuration separately defines High Speed Format 30 as **post-pass playback** rather than a source for live CRT cadence; and Apollo 15 PHO-TR155 directly places both measurements on TELMU console-09 operational indicators. Therefore TELMU is the best source-backed station-family owner for first-playable inverter electrical evidence, but missing Apollo 13 presentation evidence must not be replaced by an invented fixed channel, Apollo 15 module/indicator coordinates, LM-1 display-request numbers, LM-10 sample rates/MSKs, or Format-30 10/50-sample/s values. Exact Apollo 13 indicator/display identity, field placement, normal-format loading, live cadence, and actual PC+2 selection remain unresolved.
...
Research notes 106–121 are bounded archival refinements and corrections; they do not displace the physical-play priority.
