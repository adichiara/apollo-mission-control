...
Research note **107** revisits the onboard 77-percent rule. Apollo 13 mission material calls it a crew “thrust monitor readout” / onboard thrust criterion. LM technical documentation identifies the panel-1 dual-scale **CMD THRUST / ENG THRUST** indicator, with ENG THRUST displaying actual engine thrust as a percent from a chamber-pressure-transducer input; Apollo 13 malfunction procedures use the same indicator terminology. The historical instrument identity is therefore high-confidence, with ENG THRUST the actual-performance scale. The executable rule remains `NOT_EVALUABLE` only because the exact applicability transition after the intentionally commanded 12.6-percent and 40-percent startup segments is not yet sourced and the first playable does not fabricate a crew percent-thrust observation.

Still intentionally unresolved where evidence is insufficient: Apollo 13-specific confirmation of the 150-psi fuel-inlet mapping, exact 77-percent startup applicability gate, exact startup transient boundary for other shutdown criteria, exact alternate-inverter detail, and exact final-load ground-system internals.
...
Research notes 106–107 are bounded archival refinements, not changes to that priority ordering.
...
- exact activation point for the crew ENG THRUST <=77-percent criterion during the intentionally commanded 12.6-percent/40-percent startup sequence;
...
Research notes 095–107 and associated testing/scope documentation improve physical-run evidence quality and bounded historical interpretation but do not constitute physical validation. Physical seven-seat and five-player compact human/device PASS claims remain unmade.
