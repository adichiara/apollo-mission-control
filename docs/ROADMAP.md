...
10. Continue the Apollo 11 powered-descent/program-alarm interval as the second architecture pressure test. The guidance alarm/restart domain, landing-radar update-eligibility gate, generic independent-guidance cross-check machinery, and non-executable scenario/model-profile metadata are now implemented. Next define the actual Apollo 11 PGNS/AGS fields, tolerances, and freshness rules; upstream landing-radar measurement/quality behavior as needed; powered-descent trajectory/propulsion; estimator/controller-product interfaces; and sourced decision rules before an Apollo 11 runtime is allowed.
