...
The primary research chain now runs through notes **048–122**. Notes 098–099 establish and implement the staged final state-vector/target-load/uplink workflow; note 100 defines the backroom/SSR scope boundary; note 101 resolves immediate post-burn closure while preserving an unresolved timing-source tension; note 102 bounds spacecraft physics to decision-relevant causal fidelity; note 103 bounds MSFN/CCATS/RTCC to functional ground-data services; note 104 defines layered observation failures without random generic telemetry faults; note 105 keeps crew action explicit and scenario-authored; note 106 narrows the 150-psi ground inlet-pressure lineage toward fuel inlet / `GQ3611P` without claiming an Apollo 13-specific mapping; notes 107–108 identify the onboard ENG THRUST percent indication and bound first-playable applicability to the commanded full-throttle transition; notes 109–110 allocate the attitude start-transient exception to attitude error and preserve the lack of an Apollo 13-specific numeric duration; note 111 records the generic LM inverter convention; note 112 corrects the first-playable inverter selection from mission-specific Apollo 13 procedure evidence; note 113 closes the alternate-inverter identity; note 114 directly resolves the inverter-2-to-inverter-1 cockpit transfer sequence from the Apollo 13 LM Malfunction Procedures; note 115 closes the first-playable post-transfer timing ambiguity by requiring a fresh valid caution re-observation rather than an invented crew persistence timer; note 116 establishes the source-backed ground electrical-observation path through inverter-bus frequency `GC0155` and voltage `GC0071` PCMTEA telemetry while keeping the onboard `GL4046`/`6DS26` caution, selector position, and exact controller routing distinct; note 117 closes the first-playable direct-telemetry boundary by documenting that the reviewed primary schematics do not establish PCM telemetry taps for the derived `GL4046` caution or INV1/INV2 selector position; note 118 resolves the fixed-channel architectural ambiguity by documenting Apollo MCC dynamic display-request/channel-attach behavior; note 119 adds later-LM telemetry continuity for `GC0071V`/`GC0155F` while refusing to promote LM-10 rates/MSKs into Apollo 13; note 120 uses the mission-specific AS-508 MCC/MSFN configuration to distinguish normal LM telemetry from High Speed Format 30 post-pass playback; note 121 adds pre-/post-Apollo-13 primary continuity evidence that both inverter measurements were ground-display products and were explicitly assigned to TELMU operational indicators by Apollo 15, strengthening TELMU ownership without importing the later panel layout into AS-508; and note **122 adds mission-specific Apollo 13 transcript evidence that PC+2 activation explicitly used the onboard Power/Temp Monitor to compare inverter 2 then inverter 1, while caution/warning and telemetry availability were separately managed, establishing parallel crew-local and ground-monitoring evidence paths without inventing the unresolved TELMU layout.**

Current detailed integration roadmap: `docs/roadmap/2026-09-12_first_playable_integration.md`. Dated archival refinements are recorded under `docs/roadmap/`, including inverter selection, transfer, re-observation, ground-observation, direct-telemetry, MCC display-routing, later-LM telemetry continuity, AS-508 Format-30 boundary, TELMU indicator-continuity, and onboard/ground-monitoring records.
...
Notes 118–122 add five presentation constraints. Apollo computer-driven TV displays were dynamically requested and assigned to available channels, with channel-attach also supported; later LM-10 documentation proves `GC0071V`/`GC0155F` were operational telemetry products with format-dependent sampling and primary MSKs; the Apollo 13 AS-508 configuration separately defines High Speed Format 30 as **post-pass playback** rather than a source for live CRT cadence; Apollo 15 PHO-TR155 directly places both measurements on TELMU console-09 operational indicators; and the Apollo 13 mission transcript independently shows that the crew used the onboard Power/Temp Monitor to compare inverter 2 then inverter 1 during PC+2 activation. Therefore TELMU is the best source-backed station-family owner for first-playable ground inverter electrical evidence, while crew-local indication/report remains a distinct parallel path. Missing Apollo 13 presentation evidence must not be replaced by an invented fixed channel, Apollo 15 module/indicator coordinates, LM-1 display-request numbers, LM-10 sample rates/MSKs, or Format-30 10/50-sample/s values. Exact Apollo 13 indicator/display identity, field placement, normal-format loading, live cadence, and actual PC+2 TELMU selection remain unresolved.
...
Research notes 116–122 provide a concrete inverter example of the layered data model. Primary LM instrumentation documentation explicitly routes inverter-bus frequency `GC0155` and voltage `GC0071` through conditioning/isolation to PCMTEA telemetry and the MSFN path. The onboard derived caution `GL4046` / `6DS26` is a separate indication path, and the reviewed schematic does not show a PCMTEA telemetry tap for either that caution output or INV1/INV2 selector position. NASA TN D-7685 documents Apollo MCC computer-driven TV as a dynamic display-request/channel-attach system rather than a permanently consigned display/channel-per-console scheme. LM-10 documentation demonstrates later operational format-dependent sampling/MSK routing for the same measurement IDs, but those exact values are not Apollo 13 evidence. The March 1970 AS-508 mission configuration confirms normal LM-capable ground formats while defining High Speed Format 30 as a separate post-pass playback facility. LM-1 documentation proves both measurements were ground display/telemetry products before Apollo 13, while Apollo 15 PHO-TR155 explicitly places both on TELMU console 09 operational indicators. Finally, the Apollo 13 mission transcript independently establishes a crew-local Power/Temp Monitor comparison of inverter 2 then inverter 1 during activation and shows caution/warning power and telemetry availability as separately managed operational resources. Ground electrical telemetry therefore uses the sourced voltage/frequency measurements with TELMU as the strongest station-family owner, while exact Apollo 13 physical indicator/CRT loading remains unresolved. Selected-inverter identity, onboard caution persistence, and crew-local electrical observations remain explicit crew action/report information rather than direct ground telemetry. The exact Apollo 13 normal-format placement, live sample cadence, TELMU indicator/display format, field placement, selection key, CRT latency, actual PC+2 selection, and any Format-30 use around the burn remain unresolved.
...
`selected inverter bus → GC0155 frequency + GC0071 voltage → conditioning/isolation → PCMTEA telemetry → communications/MSFN → MCC processing → TELMU electrical evidence [exact Apollo 13 indicator/CRT loading unresolved]`

Onboard caution:

`GC0155/GC0071 failure-detection logic → GL4046 inverter caution → 6DS26 onboard indicator → crew observation/report`

Selected-inverter identity:
...
Note 118 records the MCC display-routing boundary: Apollo computer-driven TV display formats were dynamically requested and assigned to available channels, and consoles could attach to active channels. Therefore first playable does **not** invent a permanent TELMU/CONTROL channel number for inverter electrical data.

Note 119 records later-LM continuity: LM-10 primary telemetry documentation gives explicit format-dependent 1-s/s and 0.2-s/s schedules and multiple primary MSKs for `GC0071V` / `GC0155F`, but those later-mission values are not promoted into Apollo 13.

Note 120 adds the mission-specific Apollo 13 ground-format boundary. AS-508 normal network formats include LM data, but High Speed Format 30's 10/50-sample/s analog slots are explicitly part of a **post-pass playback** path selected by TICC/CEF control. Those values are therefore not a live PC+2 TELMU/CONTROL refresh rate.

Note 121 adds the station-family continuity boundary. LM-1 documentation lists both inverter measurements as explicit ground display products, and Apollo 15 PHO-TR155 puts both on TELMU console-09 operational indicators. This supports TELMU ownership for the first-playable electrical product but does **not** authorize Apollo 15 module/indicator coordinates, LM-1 display-request numbers, or later sample/MSK values as Apollo 13 facts.

Note 122 adds the flown-mission operational boundary. The Apollo 13 activation read-up explicitly powers the EPS display and directs a Power/Temp Monitor AC-bus comparison on inverter 2 then inverter 1. After PC+2, caution-and-warning power is deliberately retained until PTC is established, and later reduced-power operation continues low-bit-rate telemetry while local display functions are reduced. First playable therefore preserves crew-local indication/report and TELMU ground telemetry as parallel evidence paths rather than collapsing either into the other.

Do not invent a 1-second, 2-second, 5-second, or other post-transfer persistence timer. Do not invent a direct ground `GL4046` caution bit or selector-position telemetry. Do not invent a fixed historical TV-channel number. Do not borrow Apollo 15 TELMU panel coordinates, LM-1 display-request numbers, LM-10 sample rates/MSKs, or AS-508 Format-30 high-rate slots as Apollo 13 live PC+2 configuration. A first-playable ground electrical product may use `GC0155` / `GC0071` at TELMU; a derived electrical warning may be shown only if clearly labeled as a project-derived aid rather than a recovered Apollo 13 caution discrete. Crew-local Power/Temp Monitor and caution/warning evidence must remain crew-local unless communicated or otherwise supported by a documented telemetry path.

Still intentionally unresolved where evidence is insufficient: Apollo 13-specific confirmation of the 150-psi fuel-inlet mapping; exact operational duration/end condition of the attitude-error start transient; exact inverter-selection inhibit duration; exact AS-508/LM-7 normal-format placement and live sample cadence for `GC0071V`/`GC0155F`; exact Apollo 13 TELMU indicator/CRT identity, field placement, selection, and latency; any actual Format-30 PC+2 replay selection; whether an as-yet-unreviewed primary routing source establishes any additional caution/selector discrete; exact final-load ground-system internals; and display details whose absence does not currently block first playable.
...
Research notes 106–122 are bounded archival refinements and corrections; they do not displace the physical-play priority.
...
- exact AS-508/LM-7 normal-format placement, live sample cadence, TELMU indicator/CRT identity, field labels/placement, actual PC+2 selection, display latency, and any actual Format-30 use around PC+2;
