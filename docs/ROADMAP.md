...
2. Continue integrating the deterministic simulated-crew actor. The PC+2 ΔP shutdown path and sourced inverter-transfer contingency now use the same CAPCOM transmission → crew receipt → supported crew action pattern. The inverter path additionally preserves crew completion report → fresh post-transfer caution observation as a separate evidence step, with no fabricated dwell or automatic shutdown. Migrate additional crew actions only when their source-bounded mappings are established.
