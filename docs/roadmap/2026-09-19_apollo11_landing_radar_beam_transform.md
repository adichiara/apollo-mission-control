...
NASA's *Apollo Experience Report — Lunar Module Landing Radar and Rendezvous Radar* adds a distinct pre-serialization constraint: LR velocity data are pulse trains superimposed on a 15.3-kHz reference frequency specifically to permit determination of velocity sign; those signed measurement pulse trains, together with the range pulse train, then enter the Signal Data Converter, which uses LGC strobes to assemble/read out serial binary data. This establishes that velocity sign exists upstream of serial conversion and must be preserved by the SDC/LGC path. It does **not** identify the resulting serial word's sign bit/polarity, bit order, bias, or rounding rule.
...
1. Continue seeking LM-5/Apollo-11-effective hardware-interface evidence for **raw word length, bit order, serial sign representation/integer bias, and rounding/truncation**. The upstream velocity-sign mechanism is now documented, but its serial encoding is not. Quantity selection is closed at the flown-software boundary (`LRVELX=14`, `LRVELY=15`, `LRVELZ=16`, `LRALT=17`, octal); do not confuse these command values with returned-data encoding.
...
- **DOCUMENTED, APOLLO-PROGRAM INTERFACE:** LR velocity/range delivered to LGC as serial binary data; two binary data-flow inputs plus LGC readout/reset/range-and-velocity strobes constrain transfer framing; velocity sign is established in the measurement pulse-train path before Signal Data Converter serialization; separate pulse-train/dc-analog outputs feed LM displays.
...
- **UNRESOLVED:** per-field Apollo 11 MSK-1137 routing, exact GUIDO request workflow, MCC dynamic-data cadence/latency/freshness, LM-5 raw word length/bit order/serial sign representation/bias/rounding, and sub-second LR transition timing.
