...
A subsequent recovery pass located a public scan of the actual **LMA790-3-LM 5 and Subsequent, Volume II — Operational Procedures** in the Virtual AGC/ibiblio document collection. Its title page explicitly identifies **LM 5 and Subsequent** and states that it supersedes the 15 February 1969 issue. This closes the earlier question of whether an LM-5-specific AOH revision existed and is publicly recoverable. However, Volume II is an operational-procedures volume; targeted text checks did **not** recover an LM-5 FTP force calibration, 92.5-percent calibration, or delivered-performance value from it. The required propulsion calibration evidence therefore remains unresolved rather than being imported from later Volume-I subsystem text.

The later handbook is retained as primary **design-family/control-semantics** evidence, not LM-5 calibration evidence. The apparent 9,870/10,500 issue remains a rating/configuration question rather than an arithmetic conversion.
...
The gate remains: **a candidate PDI number is not closable merely because it appears in NASA material or numerically matches an authentic Apollo 11 mass. The source must explicitly establish Mission-G/LM-5 event identity and provenance.** The same rule applies to propulsion calibration: a later LM handbook cannot silently become LM-5 configuration evidence.
...
| --- | --- | --- |
| `thrust_n` / `end_thrust_n` | Press Kit: 9,870-lbf LM-5 mission specification; Mission Report: LM-5 FTP timing; LM-5 Volume-II AOH: configuration-specific operational procedures but no recovered force calibration; later LMA790-3-LM: 9,870-lbf nominal fixed-full-throttle / 92.5% mature-design semantics; D-7143: 10,500-lbf maximum-rated design requirement | Keep caller supplied; do not promote later-LM calibration to LM-5 or infer delivered thrust |
...
Historical validation may use **DOCUMENTED primary named-event masses + DOCUMENTED Apollo 11 preflight DPS specification + DOCUMENTED Apollo 11 FTP timing + DOCUMENTED LM-5 operational-procedure provenance + DOCUMENTED later-LM FTP semantics (configuration-qualified) + DOCUMENTED as-flown throttle/pressure telemetry + RECONSTRUCTED trajectory methodology + MODELLED unresolved PDI mass/thrust/Isp inputs**. Any scenario fixture using a convenient PDI mass or exact throttle-to-force conversion must remain MODELLED unless mission-specific evidence closes it.

## Next

Continue Mission-G PDI-mass recovery from superseded ODB pages, contractor/postflight mass ledgers, or equivalent provenance-controlled primary records. For propulsion, the generic request for an LM-5 AOH is now narrowed: **Volume II has been recovered but does not close force calibration**. Prioritize an **LM-5 Volume-I Subsystems Data revision/change page, engine acceptance/performance record, or Apollo 11 DPS Supplement 7**.

## Evidence status

**PARTIALLY DOCUMENTED.** LM-5-specific AOH Volume-II provenance is now documented and publicly recoverable, but it does not close the propulsion calibration. Primary postflight LM mass checkpoints, Apollo 11 preflight DPS limits, and Apollo 11 FTP use are documented. Exact LM-5 PDI mass, FTP force calibration, delivered thrust, and flight-effective Isp remain unresolved. Apollo 11 DPS Supplement 7 and TRW Volume II remain **BLOCKED ON NAMED SOURCE RECOVERY**.