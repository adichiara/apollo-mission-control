...
Primary interface documentation now independently bounds the physical scale transition. AC Electronics ND-1021042 (source data through 15 January 1966) identifies an LR-originated `Range low scale factor` discrete issued automatically at approximately 2,500 ft, plus digital LR data pulses under LGC readout/strobe/reset control. The LM-6 Apollo Operations Handbook (basic 15 December 1968, change 15 September 1969) preserves the same ~2,500-ft automatic scale transition and documents conversion/counting into a 15-bit serial LGC transfer. This pre/post continuity strongly supports `ALTSCBIT` as reflecting a real radar-interface scale state, while still not proving the exact LM-5 high-scale bit weight.
