...
6. Treat rule 4-9's `GUIDO 1` D/TV requirement as Mission-G-specific **minimum resource availability**, not a permanently bound channel; preserve the two GUIDO analog chart recorders as documented equipment/products without assigning unsupported powered-descent use.
7. At generic architecture level, distinguish MSK numeric format request, DRK fast pre-labeled request, and FDK out-of-tolerance format-code lookup. Do not map these to Apollo 11 GUIDO keys or program alarms without Mission-G evidence.
8. Do not infer Apollo 11 GUIDO DRK keys, exact descent display callups, or request cadence/latency from Apollo-wide/adjacent-mission architecture or prelaunch display requirements.
...
- **OPEN / MISSION-G-SPECIFIC:** GUIDO DRK labels/format mapping, FDK configuration, exact powered-descent display callups, request cadence/latency, channel identity, station-to-loop assignment, complete keyset privilege matrix, and detailed per-alarm internal call sequence.
