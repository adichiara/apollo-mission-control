...
Date: 2026-09-15
Research notes: `resources/research/137_pc2_pad_weight_epoch_boundary.md` through `resources/research/162_pc2_no_trim_ground_rule_timing.md`
...
Research note 162 adds a controller-level disposition before the crew implementation. The Flight Director narrative states that after the ~74:00 GET White Team handover, the established PC+2 ground rules included **“No PC+2 maneuver trims were required.”** The report then records the maneuver-pad update at 75:35. This proves the no-trim outcome was an explicit final-preparation ground rule, not merely an inference from the later V34/N48 procedure. The exact upstream calculation, comparison values, tolerance, and decision time remain unknown.
...
Research note 156 independently showed that the final P30 LM maneuver PAD read up at GET 077:55:24 did **not** contain a GDA trim pair. Notes 158–159 and 162 now explain that omission consistently with controller disposition, activation procedure, and software: the crew was deliberately not taken through a new Noun 48 entry.
...
1. recover a controller-side artifact **upstream of the documented no-trim ground rule** that explicitly joins the T+55 LM-burn deck family to the PC+2 comparison: calculation time/job identity, candidate trim, current/reference trim, comparison delta/tolerance, and rationale/disposition;
...
Do not force a numerical DPS model to reproduce `861.5 ft/s` by assuming either `95932 lb` or `95424.0 lb` is the exact RTCC targeting mass. Preserve provenance. For trim/GDA, model an explicit deck-selection/calculation/comparison/controller-disposition/update decision and distinguish any computed recommendation from a crew-entered Noun 48 value and from retained/observed gimbal state. Apollo 13 directly demonstrates that an RTCC mass-properties recalculation could leave a prior trim operationally unchanged after comparison; for PC+2, the Flight Director report now independently establishes a final-preparation **no-trim ground rule**, and primary operational evidence plus Apollo 13 flight-software evidence shows that disposition implemented as V34 termination at Noun 47 before Noun 48. The T+55 LM-burn deck family is directly documented, but PC+2 `calculation_time`, `job_number`, candidate trim, comparison delta/tolerance, exact retained two-axis pre-ignition gimbal state, and direct calculation-level T+55 linkage remain unknown until sourced.