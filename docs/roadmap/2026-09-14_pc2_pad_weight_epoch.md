...
12. controller-visible trim/trajectory products and cross-console reconciliation.
...
1. recover H-2 RTCC controller procedures, mass-property run sheets/listings, Flight Dynamics/RETRO worksheets, processor descriptions, weight/CG listings, or maneuver worksheets exposing **numbered mass-properties jobs near GET 55–59 hours**, the operational module/depletion accounting behind the `T+55` state, and an explicit connection from a pre-PC+2 calculation run to the accepted trim and/or final P30 module weights (`62480` / `33452`), including the precise `T+N` reference-epoch convention;
