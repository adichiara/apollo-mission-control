...
Date: 2026-09-15
Research notes: `resources/research/137_pc2_pad_weight_epoch_boundary.md` through `resources/research/157_pc2_interim_trim_reuse_free_return_burn.md`
...
Research note 152 identified the ~59-hour interim PC+2 angular GDA pair (`pitch 5.86°`, `roll 6.75°`) and CAPCOM's explicit statement that it would be updated. Research note 157 adds a crucial lifecycle constraint: the same `5.86° / 6.75°` pair was transmitted at GET 060:53 for the **actual free-return DPS burn** at TIG 061:29:42.84. Thus the pair was operationally accepted after the documented cross-console mass-properties reconciliation; it was not simply discarded. Its use for the near-term burn does not make it final for later PC+2, for which CAPCOM had explicitly promised an update.

Research note 153 establishes later PC+2 execution-state evidence. Research note 155 corrects note 154's overbroad unit reinterpretation: the April FCD LM CONTROL narrative reports the PC+2 ignition roll-GDA state in **degrees**, approximately `-2°`, a `-1.2°` change from pre-ignition, implying approximately `-0.8°` immediately before ignition. This remains execution-state evidence rather than a recovered final commanded trim product.

Research note 156 closes an incorrect archival assumption: the final P30 LM maneuver PAD read up at GET 077:55:24 did **not** contain a GDA trim pair. After attitude `272, 081`, CAPCOM explicitly said "the rest is N/A except for comments," then transmitted ullage, module weights, and throttle profile. The final targeting PAD and the missing later PC+2 trim must therefore be treated as separate products; the search should not expect the final P30 pad itself to supply the trim.
...
Keep distinct physical mass/CG, postflight reconstructed event mass, RTCC mass-properties deck/state, module/depletion state, reference epoch, generation/load time, calculation provenance, calculation run, optional job identity, processor inputs/outputs, P30 module weights, commanded angular trim, maneuver applicability, trim lifecycle/finality, observed engine-gimbal angular state, GDA linear actuator displacement, sampled/postflight actuator trace, ignition compliance response, and product lifecycle/finality.

Do not infer the cause of the 508-lb difference; assign job 27 to PC+2; claim a numbered job or direct mathematical `T+55` derivation for `5.86° / 6.75°`; treat the pair as discarded merely because PC+2 required a later update; treat it as final for PC+2 merely because it was used for the free-return burn; treat generic 3°/in as exact LM-7 calibration; substitute postflight actuator displacement for the missing final commanded angular trim; or assume the final P30 targeting PAD carried a GDA trim field.

## Next unresolved numerical inputs

Priority order:

1. recover an H-2 CONTROL/Flight Dynamics working sheet, Flight Director Log entry, GDA setup/checklist record, telemetry/strip-chart annotation, or mass-properties job output between the 61:29 free-return burn and 79:27 PC+2 giving the later PC+2 **commanded angular trim** and its provenance/job number;
2. recover a direct record tying the accepted `5.86° / 6.75°` free-return trim to a specific `T+55` calculation run/job, rather than only to the surrounding deck/reconciliation narrative;
3. recover LM-7-specific GDA/engine acceptance calibration if a higher-fidelity degrees↔inches transform is needed;
4. recover the operational module/depletion accounting behind `T+55` and the precise `T+N` reference-epoch convention;
...
Do not force a numerical DPS model to reproduce `861.5 ft/s` by assuming either `95932 lb` or `95424.0 lb` is the exact RTCC targeting mass. Preserve provenance. For trim/GDA, require explicit representation, units, maneuver applicability, and lifecycle state. The documented generic hardware scale is ±2 in ↔ ±6°, but exact LM-7 calibration remains unknown. The final P30 pad is positively documented as omitting GDA trim. The `5.86° / 6.75°` pair is positively documented as accepted and used for the 61:29 free-return burn while remaining update-required for later PC+2. PC+2 `job_number`, complete later commanded angular trim, and explicit calculation-level `T+55` linkage remain unknown until directly sourced.