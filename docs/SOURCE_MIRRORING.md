...
Extracted pages are stored as `resources/primary-sources/files/<id>-pages.pdf`, and the manifest's `pages` field records which section each claim rests on.

**Extraction is a manual step.** `fetch` does not download or split these entries. It prints the target path and the page description and moves on, because deciding which pages answer a claim is a research judgment, and because the source documents are too large to pull just to discard most of them. Populate this tier by hand, then `verify` hashes the result like any other file.
...
The manifest carries a `rights` field so each entry records the basis rather than leaving it implied.

**The rule is absolute: `rights: unclear` means `tier: link-only`.** A note explaining the doubt does not license mirroring the file. `tests/test_source_manifest.py` enforces this, so the policy cannot drift from the manifest without failing the suite.

The cost of being wrong in the conservative direction is one citation a reader must fetch themselves. The cost in the other direction is redistributing material the project has no established right to redistribute. Those are not symmetric.

This is an engineering judgment recorded for review, not legal advice. The one entry currently affected is the ITC 1970 conference paper: it is genuinely useful INCO evidence, but it is a conference proceeding rather than a NASA publication, so it stays `link-only` until someone confirms the proceedings' terms.
...
`fetch` downloads Tier 1 entries only. It refuses to write a file over 100 MB and warns over 50 MB, naming the entry to reclassify as `pages`; it skips files already present, so re-running is cheap; and for Tier 2 entries it prints the manual extraction instruction rather than downloading anything. It writes no lock file when nothing was fetched.
