...
- Recovered the pre-61:29 checkout observation `within about 0.3` / `plenty close`; **note 179 now strengthens its classification**: the Flight Director loop explicitly places it inside CONTROL's gimbal-trim checkout and readiness decision immediately before the 61:29 burn. It is an operational preburn gimbal-checkout acceptance statement, not the missing PC+2 ground-computation comparison criterion.
...
- **New in note 179:** recovered the context around CONTROL's `within about 0.3` / `plenty close` statement. CONTROL explicitly directs the gimbal-trim procedure, reports `Trim looks okay`, answers FLIGHT's question about closeness with `within about 0.3`, and then declares readiness. This establishes a spacecraft checkout acceptance layer separate from the unresolved computational comparison layer.
- Kept commanded trim, checkout tolerance, post-compliance actuator state, and later `5.85 / 6.74` PC+2 reference readback as separate provenance layers.

## Next

Search Apollo 13 archival/controller material specifically for a T+55 generation/load record or downstream RTCC/RTACF LM-burn run/request/output that bridges the T+55 deck to the ~59 GET trim solution. Highest-value fields remain the real-time T+55 weight/c.g. values, CONTROL's alternative numerical trim, explicit T+55-to-`5.86 / 6.75` run linkage, **PC+2 ground-computation** comparison/acceptance basis, and calculation/job identity. Also seek telemetry or a controller working sheet that records the exact two-axis GDA state after 61:29 powered-flight compliance. Do not reuse the `~0.3°` preburn checkout closeness or T+25 `0.01°` no-update result as that missing criterion.