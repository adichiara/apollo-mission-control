...
- Research note 155 subsequently corrected the unit interpretation: the LM CONTROL narrative's approximately `-2°` roll-GDA state and `-1.2°` change are angular values, implying approximately `-0.8°` pre-ignition. The separate Mission Report maneuver-performance table remains linear actuator displacement in inches.
...
The simulator must treat **commanded angular trim**, **observed angular engine-gimbal/GDA state**, and **linear GDA actuator displacement** as different typed quantities with explicit units and provenance. The final ~78-hour commanded angular trim pair is still missing.
