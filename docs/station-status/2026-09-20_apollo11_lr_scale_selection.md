...
NASA's *Apollo Experience Report — Lunar Module Landing Radar and Rendezvous Radar* further states that LR velocity measurements are pulse trains superimposed on a 15.3-kHz reference frequency to permit determination of sign, and that these velocity pulse trains plus the range pulse train feed the Signal Data Converter before serial-binary transfer to the LGC. This resolves the location of one sign function: velocity sign exists upstream of serialization. It does not establish how that sign is represented in the serial word.

Flown LUMINARY 099 closes the software-side quantity-select mapping as well. In `P20-P25.agc`, the radar lead-ins invoke `INITREAD` with octal `14` for `LRVELX`, `15` for `LRVELY`, `16` for `LRVELZ`, and `17` for `LRALT`. These are Apollo-11-effective read-selection commands, not evidence for the numeric encoding of the returned serial data. Bit order, serial sign convention, integer bias, raw word length, and rounding/truncation therefore remain unresolved.
...
A spacecraft model may preserve LR altitude scale state internally, use 1.079 ft/count low scale and the directly derived 5.395 ft/count high scale, encode the Apollo 11 mission load as selecting radar-performed slant-range Doppler compensation, preserve the flown LGC quantity-selection mapping (`Vx/Vy/Vz/range` = octal `14/15/16/17`), and preserve the architectural fact that velocity sign is determined before Signal Data Converter serialization. These are onboard/interface facts; they do not establish a controller-visible control or the unresolved raw numerical coding. Do not present 2,500 ft as an exact invariant Apollo-11 switching altitude.
...
- **DOCUMENTED INTERFACE:** LR uses separate `LR in "1"`/`LR in "0"` binary data flow and LGC-controlled readout/reset/range/Vx/Vy/Vz strobes; velocity sign is determined in the measurement pulse-train path before serial conversion.
...
- **UNRESOLVED:** LM-5 raw word length, bit order, serial sign representation/bias, rounding/truncation, and controller-visible compensation/scale-state consequences.
