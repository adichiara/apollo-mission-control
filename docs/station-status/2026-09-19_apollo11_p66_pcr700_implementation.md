...
Research 330 also recovers a historical R-567 Section 5 **Guidance Equations (Revision 4)** control-index sheet from the surviving cumulative Revision 11 artifact. This narrows technical retrieval but does not itself prove the Apollo 11 flight-effective Section 5 page set.

## Station-facing boundary

Neither finding establishes a new GUIDO, CONTROL, FLIGHT, or other MCC player product. The Revision 4 index also does not establish a ground display cadence or prove the January SCB proposal's once-per-second HDOT display behavior in the final flight configuration.
