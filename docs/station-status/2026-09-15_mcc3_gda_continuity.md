...
Primary NASA voice also establishes that the same `5.86 / 6.75` pair was passed and accepted as the commanded GDA pair for the 61:29 free-return DPS burn. This matters because CONTROL later says the 40%-thrust compliance during that burn was expected to leave the GDA state optimum for PC+2.
...
**Improved.** The Flight Control Division Mission Operations Report explicitly states that it uses only data available in real time. Its chronology explicitly labels the T+55 update **"RTCC (LM burn) mass property decks."** This is direct mission-specific evidence that T+55 belongs to the RTCC LM-burn processing domain.

The same chronology's earlier T+25 case explicitly connects an RTCC mass-properties run to pitch/yaw trim comparison against T+6. Together these establish a mission-specific workflow/domain link: time-tagged RTCC mass properties can feed/evaluate LM-burn P/Y trims. The `0.01°` T+25 result remains scoped only to that earlier no-update decision.
...
Still unresolved: T+55 weight/c.g./deck contents and generation/load time, CONTROL's competing numerical values, PC+2-specific acceptance criterion, RTCC/RTACF job/request identity, direct T+55-to-`5.86 / 6.75` data lineage, and exact two-axis post-61:29 complied GDA state.

## Simulator constraint

Represent mass-properties `reference_epoch`, `processing_domain`, generation/load time, job/run identity, and consuming trim solution as separate provenance fields. For T+55, only the reference epoch and RTCC LM-burn domain are currently source-backed. Likewise keep ground-computed trim, commanded/loaded burn trim, post-powered-flight complied actuator state, and later pad/reference values separate.