...
46. **RESOLVED for first-playable inverter identity; exact transfer procedure remains unresolved:** Apollo 13's mission-specific PC+2 read-up fixes the active source as inverter 2 by closing `CB(16) INVERTER 2` and scratching the stock `Select Inverter 1` step. The contemporaneous shutdown rule then requires trying to switch inverters before treating a continuing inverter light as a shutdown condition. The primary LM Operations Handbook documents only two redundant LM inverters, so from the source-bounded inverter-2 starting state the other inverter identity is necessarily inverter 1. First playable may therefore represent the contingency identity as `inverter 2 → inverter 1`, but must not invent the exact switch/circuit-breaker chronology, reset steps, persistence dwell, crew-member assignment, or ground visibility of selector position. See research notes 112–113; note 111 is superseded in part.
