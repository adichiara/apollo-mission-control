...
46. Can the actual **Mission H-2 data-pack Revision N**, its transmittal/index, or a **Requirements and Configuration data-pack definition/key** be recovered to establish which named configuration inputs changed at the PHO-TR155 Revision-C milestone and whether any contained TELMU loading? PHO-TR515 establishes that data-pack circulation fed the Requirements and Configuration process that prepared PHO-TR155; PHO-TR460 shows multiple named H-2 pack products. Research note 128 adds that `FCOB` and `LSB` are directly attested contemporary organizational acronyms for Flight Control Operations Branch and LM Systems Branch, while contemporary MSC records also identify CSM Systems, Flight Dynamics, Mission Control Requirements, and Experiments Systems branches. These matches are archival search leads only: the PHO-TR460 pack labels remain unexpanded unless a direct pack key/transmittal/cross-reference is recovered. Revision N therefore must not be treated as PHO-TR155 itself or as TELMU-specific without the missing mapping. See research notes 126–128.
47. Can the **Mission H-2 Flight Control Data Requirements Document**, Flight Control branch requirement letters, or recorder cross-reference/overlay requirements be recovered? PHO-TR515 establishes that these source classes could carry console number, telemetry measurement/subformat, high-speed format/subformat, title, and recorder pen-location assignments. Recovery could test whether `GC0071V` / `GC0155F` were routed to a TELMU recorder product without assuming that any PHO-TR460 data-pack abbreviation is the same document. See research note 129.
