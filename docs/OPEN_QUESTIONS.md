...
46. **RESOLVED for initial PC+2 inverter selection; post-warning named alternate remains unresolved:** Apollo 13's mission-specific PC+2 read-up closes `CB(16) INVERTER 2` and explicitly scratches the stock `Select Inverter 1` step, overriding the generic handbook convention used in note 111. The first-playable pre-burn AC source is therefore inverter 2. The shutdown rule still requires trying a redundant-inverter switch and observing whether the warning remains, but no reviewed mission-specific rule yet names the post-warning alternate as inverter 1. Do not hard-code a named inverter-2-to-inverter-1 contingency action until direct procedure evidence is recovered. See research note 112; note 111 is superseded in part.
