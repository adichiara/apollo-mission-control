...
46. **RESOLVED for first-playable inverter transfer and timing semantics:** Apollo 13's mission-specific PC+2 read-up fixes the active source as inverter 2. The Apollo 13 *LM Malfunction Procedures* gives the alternate-selection sequence for inverter 2 operating: close `CB(11) EPS: INV 1`, set `INVERTER` to `1`, open `CB(16) EPS: INV 2`, then evaluate whether the INVERTER caution has cleared. The flowchart specifies no numeric crew persistence dwell. NASA instrumentation documentation for LM-5-and-subsequent describes inverter-selection transient-inhibit logic, so first playable treats the post-switch decision as a **fresh valid caution re-observation**, not an invented timer. If the fresh valid caution remains, the contemporaneous PC+2 shutdown rule is satisfied. Exact inhibit duration, crew-member assignment, controller voice wording, selector-position ground visibility, and exact TELMU/CONTROL display field remain unresolved. See research notes 112–115; note 111 is superseded in part.
