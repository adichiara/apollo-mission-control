...
45. **RESOLVED for operational first-playable rule allocation:** the PC+2 “start transient” exception applies to the ±10-degree **attitude-error** shutdown criterion, not the ±10-degree/sec rate criterion, because the contemporaneous CAPCOM transmission and Haise readback both state the rule that way. The later Mission Operations Report wording that attaches the exception to rate is retained as conflicting postflight evidence. The exact start/end duration of the transient remains unresolved and must not be inferred from the 5-second, 21-second, or +26-second thrust-profile landmarks. See research note 109.
