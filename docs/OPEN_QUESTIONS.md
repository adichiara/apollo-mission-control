...
46. **RESOLVED for first-playable inverter transfer procedure:** Apollo 13's mission-specific PC+2 read-up fixes the active source as inverter 2. The Apollo 13 *LM Malfunction Procedures* INVERTER caution flowchart then directly gives the alternate-selection sequence for inverter 2 operating: close `CB(11) EPS: INV 1`, set `INVERTER` to `1`, open `CB(16) EPS: INV 2`, then re-observe whether the INVERTER caution has cleared. If the caution remains, the contemporaneous PC+2 shutdown rule is satisfied. No numeric persistence dwell, crew-member assignment, exact controller voice wording, independent ground visibility of selector position, or exact TELMU/CONTROL display field is recovered. See research notes 112–114; note 111 is superseded in part.
